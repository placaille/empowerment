...
class MineReproDiscreteStaticAgent(object):
    ...
    def val_emp_train_step(self):
        batch = self.memory.sample_data(self.max_batch_size)

        obs_start = torch.FloatTensor(batch.obs_start)
        trgt_emp = torch.FloatTensor(batch.empowerment)
        pred_emp = self.model_empowerment(obs_start.to(self.device)).squeeze(1)

        loss_empowerment = self.obj_empowerment(pred_emp, trgt_emp)

        self.optim_empowerment.zero_grad()
        loss_empowerment.backward()
        self.optim_empowerment.step()

        return loss_empowerment.item()
